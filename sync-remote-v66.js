...
function renderRemote(){
  ...
  controls.innerHTML=`
    <div class="remote-controls-grid">
      ${cricketNumbers.filter(n=>n!=="BULL").map(n=>`<button class="target-button" data-target="${n}">${n}</button>`).join("")}
    </div>
    <div class="bull-row">
      <button class="target-button bull-button" id="bull-btn">BULL</button>
      <button class="target-button bullseye-button" id="bullseye-btn">BULL'S EYE</button>
    </div>
  ...
  document.getElementById("bull-btn").addEventListener("click",()=>addMarks("BULL",1,false));
  document.getElementById("bullseye-btn").addEventListener("click",()=>addMarks("BULL",2,true));
...
function attachTargetHandlers(){
  document.querySelectorAll(".target-button[data-target]").forEach(button=>{
...
function openMultiplierMenu(button){
  ...
  const rect=button.getBoundingClientRect(),width=160;
  ...
  menu.querySelectorAll("button").forEach(opt=>opt.addEventListener("click",async e=>{
    e.stopPropagation();
    await addMarks(number,Number(opt.dataset.hit),false);
    closeMenus();
  }));
}
...
function evaluateRoundLimit(){
  // Tur limiti bittiğinde değerlendirme yalnızca iki aşamalıdır:
  // 1) En çok başarılı atış
  // 2) En yüksek puan
  // İkisi de eşitse, eşit kalan oyuncuların TAMAMI ani ölüme gider.
...
function startSuddenDeath(indices){
  const contenders=[...new Set(indices.map(Number))]
    .filter(i=>Number.isInteger(i) && i>=0 && i<game.players.length);

  if(contenders.length===1) return finishWinner(contenders[0],"Eşitlik bozma");

  game.suddenDeath={
    active:true,
    round:1,
    contenders:[...contenders],
    scores:Object.fromEntries(contenders.map(i=>[i,0])),
    completed:[],
    message:"3 dartlık ani ölüm"
  };
  game.activePlayer=contenders[0];
