...
function renderBoard() {
    ...
    // HEDEF sütunu oyuncuların arasına yerleşir.
    // 2: 1|HEDEF|1, 3: 2|HEDEF|1, 4: 2|HEDEF|2,
    // 5: 3|HEDEF|2, 6: 3|HEDEF|3
    ...
    players.slice(0, leftCount).forEach((player, i) => {
        html += `
    ...
    players.slice(leftCount).forEach((player, j) => {
        const i = leftCount + j;
        html += `
            <th class="${i === activePlayer ? "active-player" : ""}">
                ${i === activePlayer ? "▶ " : ""}
    ...
    cricketNumbers.forEach(number => {
        ...
        players.slice(0, leftCount).forEach(player => {
            html += `
                <td class="mark-cell">
        ...
        players.slice(leftCount).forEach(player => {
            html += `
                <td class="mark-cell">
                    <div class="mark-symbol-wrap">${markSymbol(player.marks[number])}</div>
                </td>
