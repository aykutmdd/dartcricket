...
function openMultiplierMenu(button){
  ...
  menu.innerHTML=`<button data-hit="2">D${number}</button><button data-hit="3">T${number}</button>`; document.body.appendChild(menu);
  ...
  menu.querySelectorAll("button").forEach(opt=>opt.addEventListener("click",async e=>{ e.stopPropagation(); await addMarks(number,Number(opt.dataset.hit)); closeMenus(); }));
}
...
function recordStats(number,hits){
  const p=game.players[game.activePlayer];
  p.successfulThrows=Number(p.successfulThrows||0)+1;
  if(number==="BULL"){
    p.bullHits=Number(p.bullHits||0)+hits;
    if(hits>=2) p.bullseyes=Number(p.bullseyes||0)+1;
  }
}
function shotLabel(number,hits){ return `${hits===1?"S":hits===2?"D":"T"}${number}`; }

async function addMarks(number,hits){
  if(!game || game.winner) return;
  if(game.suddenDeath?.active) return addSuddenDeathHit(number,hits);
  saveState();
  recordStats(number,hits);
  let current=Number(game.players[game.activePlayer].marks[number]||0), newTotal=current+hits;
  game.currentTurnShots=[...(game.currentTurnShots||[]),shotLabel(number,hits)].slice(-3);
...
async function addSuddenDeathHit(number,hits){
  saveState();
